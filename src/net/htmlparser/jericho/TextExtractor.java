...
public class TextExtractor implements CharStreamSource {
	...
	private final class Processor {
		...
		/**
		 * yosef binjamini
		 * my implementation to get the index of the original string and the plain text string
		 */
		...
		public String toString() {
			...
			for (NodeIterator nodeIterator=new NodeIterator(segment); nodeIterator.hasNext();) {
				...
				if (segment instanceof Tag) {
						...
						if (includeAttributes) {
							for (Attribute attribute : startTag.getAttributes()) {
								if (includeAttribute(startTag,attribute)){
									...
									indexer.add(new parlelIndex(++htmlIndex, ++plainTextIndex, ' '));
									for (int i = 0; i < attribute.getValueSegment().length(); i++) {
										indexer.add(new parlelIndex(++htmlIndex, ++plainTextIndex, attribute.getValueSegment().charAt(i)));
									}
									indexer.add(new parlelIndex(++htmlIndex, ++plainTextIndex, ' '));
					...
					for (int i = 0; i < segment.length(); i++) {
						indexer.add(new parlelIndex(++htmlIndex, ++plainTextIndex, segment.charAt(i)));
					}
				}
			}
			 ...
			 * my implementation to get the index of the original string and the plain text string
			 */
//			for (Iterator iterator = indexer.iterator(); iterator.hasNext();) {
//				parlelIndex parlelIndex = (parlelIndex) iterator.next();
//				if (parlelIndex.currentChar==' ') {
//					parlelIndex = (parlelIndex) iterator.next();
//					if (parlelIndex.currentChar==' ') {
//						indexer.remove(parlelIndex);
//						for (Iterator iterator2 = iterator; iterator2
//								.hasNext();) {
//							parlelIndex parlelIndex2 = (parlelIndex) iterator2
//									.next();
//							parlelIndex2.htmlIndex--;
//							parlelIndex2.plainTextIndex = (parlelIndex2.currentChar=='-')?parlelIndex2.plainTextIndex:parlelIndex2.plainTextIndex-1;
//							
//						}
//						
//					}
...
//			}
	...
	public Marker getMarker(int index, int length) {
		if (marker == null) {
			Processor p = new Processor(segment, convertNonBreakingSpaces, includeAttributes, excludeNonHTMLElements);
